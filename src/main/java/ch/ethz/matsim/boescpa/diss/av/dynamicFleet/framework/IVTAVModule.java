...
public class IVTAVModule extends AbstractModule {
    final static public String AV_MODE = "av";
    final static Logger log = Logger.getLogger(IVTAVModule.class);

	@Override
	public void install() {
        addRoutingModuleBinding(AV_MODE).to(AVRoutingModule.class);
        bind(ScoringFunctionFactory.class).to(IVTAVScoringFunctionFactory.class).asEagerSingleton();
        ...
        bind(TravelTime.class).annotatedWith(Names.named(IVTAVModule.AV_MODE))
                .to(Key.get(TravelTime.class, Names.named(VrpTravelTimeModules.DVRP_ESTIMATED)));

        bind(VehicleType.class).annotatedWith(Names.named(IVTAVModule.AV_MODE)).toInstance(VehicleUtils.getDefaultVehicleType());
	...
	@Provides @Named(IVTAVModule.AV_MODE)
    LeastCostPathCalculator provideLeastCostPathCalculator(@Named(IVTAVModule.AV_MODE) Network network, @Named(VrpTravelTimeModules.DVRP_ESTIMATED) TravelTime travelTime) {
