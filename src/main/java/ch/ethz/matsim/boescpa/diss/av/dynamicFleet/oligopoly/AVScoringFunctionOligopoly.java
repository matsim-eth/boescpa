...
public class AVScoringFunctionOligopoly implements SumScoringFunction.ArbitraryEventScoring {
    final static Logger log = Logger.getLogger(AVScoringFunctionOligopoly.class);
	...
	private final double votCorrectionFactor;
    ...
    public AVScoringFunctionOligopoly(AVConfig config, Person person, double marginalUtilityOfMoney, double marginalUtilityOfTraveling, double votCorrectionFactor) {
        ...
        this.votCorrectionFactor = votCorrectionFactor;
        this.config = config;
        this.person = person;
    }
    ...
    private double computePricingScoring(AVScoringTrip trip) {
        ...
        if (priceStructure != null) {
            ...
            costs += (billableTravelTime / 60.0) * priceStructure.getPricePerMin() * votCorrectionFactor;
