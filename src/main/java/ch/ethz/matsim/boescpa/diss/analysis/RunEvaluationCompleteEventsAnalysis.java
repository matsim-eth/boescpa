 ...
 * @author boescpa
 */
public class RunEvaluationCompleteEventsAnalysis {
	...
	private RunEvaluationCompleteEventsAnalysis(String pathToRunFolders, String simClass, double scaleFactor) throws IOException {
	...
	public static void main(final String[] args) throws IOException {
		...
		RunEvaluationCompleteEventsAnalysis runEvaluation = new RunEvaluationCompleteEventsAnalysis(pathToRunFolders, simClass, scaleFactor);
	...
	private String getAnzPassengers(String pathToRunFolder, String runId) throws IOException {
		BufferedReader reader = IOUtils.getBufferedReader(pathToRunFolder + File.separator
				+ runId + ".output_events.xml.gz_analysisResultsTargetFunction.csv");
		String line = reader.readLine();
		while (!line.contains("Passenger Kilometers and number of passengers:")) line = reader.readLine();
		Map<String, Double> evalResult = new HashMap<>();
		line = reader.readLine();
		while (!line.equals("")){
			evalResult.put(line.split(";")[0],Double.parseDouble(line.split(";")[2]));
			line = reader.readLine();
		}
	...
	private String getPassengerKM(String pathToRunFolder, String runId) throws IOException {
		...
		line = reader.readLine();
		while (!line.equals("")){
			evalResult.put(line.split(";")[0],Double.parseDouble(line.split(";")[1]));
			line = reader.readLine();
		}
		String out = evalResult.keySet().contains("pt_bus") ? DEL + df.format(evalResult.get("pt_bus")) : DEL + 0;
		out = evalResult.keySet().contains("pt_other") ? out + DEL + df.format(evalResult.get("pt_other")) : out + DEL + 0;
	...
	private String getVKT(String pathToRunFolder, String runId) throws IOException {
		BufferedReader reader = IOUtils.getBufferedReader(pathToRunFolder + File.separator
				+ runId + ".output_events.xml.gz_analysisResultsTargetFunction.csv");
		String line = reader.readLine();
		while (!line.contains("Vehicle Kilometers:")) line = reader.readLine();
		Map<String, Double> evalResult = new HashMap<>();
		line = reader.readLine();
		while (!line.equals("")){
			evalResult.put(line.split(";")[0],Double.parseDouble(line.split(";")[1]));
