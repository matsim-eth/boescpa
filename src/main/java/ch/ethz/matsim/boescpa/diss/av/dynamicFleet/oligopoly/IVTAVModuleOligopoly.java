...
public class IVTAVModuleOligopoly extends AbstractModule {
    final static public String AV_MODE = "av";
    final static Logger log = Logger.getLogger(IVTAVModuleOligopoly.class);

	@Override
	public void install() {
        addRoutingModuleBinding(AV_MODE).to(AVRoutingModule.class);
        bind(ScoringFunctionFactory.class).to(IVTAVScoringFunctionFactoryOligopoly.class).asEagerSingleton();
        ...
        bind(TravelTime.class).annotatedWith(Names.named(IVTAVModuleOligopoly.AV_MODE))
                .to(Key.get(TravelTime.class, Names.named(VrpTravelTimeModules.DVRP_ESTIMATED)));

        bind(VehicleType.class).annotatedWith(Names.named(IVTAVModuleOligopoly.AV_MODE)).toInstance(VehicleUtils.getDefaultVehicleType());
        ...
        addControlerListenerBinding().to(Key.get(ParallelLeastCostPathCalculator.class, Names.named(IVTAVModuleOligopoly.AV_MODE)));
        addMobsimListenerBinding().to(Key.get(ParallelLeastCostPathCalculator.class, Names.named(IVTAVModuleOligopoly.AV_MODE)));

        bind(Network.class).annotatedWith(Names.named(DvrpModule.DVRP_ROUTING)).to(Network.class);
        bind(Network.class).annotatedWith(Names.named(IVTAVModuleOligopoly.AV_MODE)).to(Key.get(Network.class, Names.named(DvrpModule.DVRP_ROUTING)));
	}

	@Provides @Singleton @Named(IVTAVModuleOligopoly.AV_MODE)
	...
	@Provides @Named(IVTAVModuleOligopoly.AV_MODE)
    LeastCostPathCalculator provideLeastCostPathCalculator(@Named(IVTAVModuleOligopoly.AV_MODE) Network network, @Named(VrpTravelTimeModules.DVRP_ESTIMATED) TravelTime travelTime) {
