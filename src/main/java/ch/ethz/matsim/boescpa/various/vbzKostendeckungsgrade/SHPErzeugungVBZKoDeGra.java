 ...
 * @author boescpa
 */
public class SHPErzeugungVBZKoDeGra {

	public static void main(String[] args) throws Exception {
		final String pathToNetwork = args[0];
		final String pathToSchedule = args[1];
		final String pathToKoDeGraCSV = args[2];
		final String pathToOutputFolder = args[3] + File.separator;
		final String coordinateSystem = args.length > 4 ? args[4] : "EPSG:2056"; // EPSG-Code for Swiss CH1903_LV03+
		...
		BufferedReader reader = IOUtils.getBufferedReader(pathToKoDeGraCSV);
		...
		PolylineFeatureFactory lineFactory = new PolylineFeatureFactory.Builder().
				...
				addAttribute("KoDeGra_%", Double.class).
				create();
		String line = reader.readLine();
		while (line != null) {
			Id<TransitLine> transitLineId = ScheduleFilterungVBZKoDeGra.getTransitLineId(schedule, line);
			...
			SimpleFeature ft = lineFactory.createPolyline(
					...
					new Object[]{transitLine.getId().toString(),
							...
							Double.parseDouble(line.split(DatenaufbereitungVBZKoDeGra.DEL)[2].replace("%", ""))
					}, null);
			features.add(ft);
			line = reader.readLine();
		}
